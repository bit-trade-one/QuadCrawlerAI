...
ws = create_connection("ws://192.168.1.133:54323")
    ...
    if joy_msg.buttons[1]== 1:
       ...
       if lx > 0:
           ...
           if (xSpeed != spcom(xspd)):
               xSpeed = spcom(xspd)
               Move_Cntrl(CW,spcom(xspd))
               State = CW
       else:
           print ("CCW")
           if (xSpeed != spcom(xspd)):
               xSpeed = spcom(xspd)
               Move_Cntrl(CCW,spcom(xspd))
               State = CCW
    elif lx == 0:
       if ly > 0:
           print("FW")
           if (ySpeed != spcom(yspd)):
               ySpeed = spcom(yspd)
               Move_Cntrl(FW,ySpeed)
               State = FW
       else:
           print("RW")
           if (ySpeed != spcom(yspd)):
               ySpeed = spcom(yspd)
               Move_Cntrl(RW,spcom(yspd))
               State = RW
    else:
       pass
...
rospy.init_node('Joy2qc')
rospy.Subscriber('joy', Joy, joy_contrl)
rospy.spin()
